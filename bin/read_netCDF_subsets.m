function datasub = read_netCDF_subsets(x0, y0, radius, t0, trange, datadir, opt)
% purpose:
...
%     end
    ...
    NCFN.default = [datadir filesep ProdName.default filesep '2020' filesep  folderN filesep ...
                    'GOES_' channelID '_8N-18N-62W-50W_' datestr(floor(tnow),format2) '.nc'];
                
   url0 = [datadir filesep ProdName.backup filesep '2020' filesep folderN filesep 'catalog.html'];
   url_thredds  = replace(url0, 'dodsC','catalog');
    ...
    fileid_full = get_fileID_from_thredds_catalog(url_thredds, fileid);
    %fileid_full = get_fileID_from_thredds_catalog(url_new, fileid);
    
%     NCFN.backup = [datadir filesep ProdName.backup filesep '2020' filesep folderN filesep ...
%                    'clavrx_OR_ABI-L1b-RadF-M6C01_G16_s' fileid_full '_BARBADOS-2KM-FD.level2.nc'];
   ...
   % then read another datasource:
    ...
    if default_flag
        ...
        datasub_tmp = take_subsets_from_secondary_source(NCFN.backup, x0(it), y0(it), radius, channel);
